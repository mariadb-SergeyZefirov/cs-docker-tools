...
CREATE TEMP TABLE text_parts (text_part TEXT);
...
INSERT INTO text_parts (text_part) VALUES ("ещё т дремлешь, друг прелестный!");
...
INSERT INTO texts (num_delims, text_line)
    SELECT 1, 'START' || T0.text_part || '1' || T1.text_part || '2' || T2.text_part || 'END'
    FROM text_parts as T0, text_parts as T1, text_parts as T2;

INSERT INTO texts (num_delims, text_line)
    SELECT 1, 'START' || T0.text_part || '1' || T1.text_part || '2' || T2.text_part || '3' || T3.text_part || 'END'
